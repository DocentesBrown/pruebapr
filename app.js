...
function Button({ onClick, children, variant = 'primary', className = '' }) {
  ...
  return e('button', {
    onClick,
    className: `px-4 py-2 rounded-xl font-semibold shadow-sm transition-opacity hover:opacity-90 ${className}`,
...
function NewStudentModal({ open, course, onClose, onAdd }) {
  ...
  function submit(ev) {
    ...
    onAdd({ id: uid('st'), name, subjects_enrolled: enrolled, external_subjects: {}, grades: {} });
    onClose();
  }
  ...
  return e('div', { className: 'max-w-4xl mx-auto p-4 md:p-6' },
    e('header', { className: 'flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-white p-4 rounded-3xl shadow-sm border border-slate-200' },
      ...
      e('div', { className: 'flex gap-2' },
        ...
        e(Button, { variant: 'secondary', onClick: () => setNewCourseOpen(true) }, '+ Nuevo Curso')
      )
    ),
          ...
          e('tbody', null,
            Object.values(course.students || {}).map(st => {
              ...
              return e('tr', { key: st.id, className: 'border-b hover:bg-slate-50 transition-colors' },
                ...
                e('td', { className: 'p-3 text-center flex justify-center gap-2' },
